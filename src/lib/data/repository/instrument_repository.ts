...
import {
	InstrumentTable,
	mapHitDtoToHitTypeWithId,
	mapHitTypeToHitDto,
	mapInstrumentDtoToInstrumentWithId,
	mapInstrumentToInstrumentDto
} from '$lib';
...
export class InstrumentRepository {
	...
	async saveInstrument(instrument: InstrumentWithId): Promise<void> {
		// Save HitDto's first
		const hitDtos: HitDto[] = [...instrument.hitTypes.values()].map((hitType) =>
	...
	async deleteInstrument(id: string): Promise<void> {
		...
		await Promise.all(
			Array.from(instrument.hitTypes.keys()).map((hitId) => this.hitTable.deleteHit(hitId))
		);
